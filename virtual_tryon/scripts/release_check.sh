...
if [[ -d backend && -f backend/requirements.txt ]]; then
  if command -v "$PYTHON_BIN" >/dev/null 2>&1 || [[ -x "$PYTHON_BIN" ]]; then
    ...
    warn_check "Backend mock tests" "Python executable '$PYTHON_BIN' is unavailable; backend checks skipped"
    warn_check "Evaluation set validation" "Python executable '$PYTHON_BIN' is unavailable; validation skipped"
  fi
else
  warn_check "Backend mock tests" "backend directory or requirements file is missing; backend checks skipped"
